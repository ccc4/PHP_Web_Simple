<?php
 ...
 ?>
  ...
  if($page != 1){
    $paging .= '<a href="./index.php?id=guest&page=1">처음</a>';
  }else if($page = 1){
    $paging .= '<strong class="">처음</strong>';
  }
  // 첫 섹션이 아니라면 이전 버튼을 생성
  if($currentSection != 1) {
    $paging .= '<a href="./index.php?id=guest&page='.$prevPage.'">이전</a>';
  ...
  for($i = $firstPage; $i <= $lastPage; $i++){
    if($i == $page){
      ...
      $paging .= '<a href="./index.php?id=guest&page='.$i.'">'.$i.'</a>';
    }
  }

  // 마지막 섹션이 아니라면 다음 버튼을 생성
  if($currentSection != $allSection){
    $paging .= '<a href="./index.php?id=guest&page='.$nextPage.'">다음</a>';
  }else if($currentSection = $allSection){
    $paging .= '<strong class="">다음</strong>';
  }
  // 마지막 페이지가 아니라면 끝 버튼을 생성
  if($page != $allPage){
    $paging .= '<a href="./index.php?id=guest&page='.$allPage.'">끝</a>';
  ...
  $sql = 'SELECT * FROM guest ORDER BY idx ASC'.$sqlLimit;
